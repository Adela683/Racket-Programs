...
(define (cst-func suffixes)
  (let* ((prefix (longest-common-prefix-of-list suffixes))
         ...
         (new-suffixes (map (lambda (suf) (drop suf prefix-length)) suffixes)))
...
(define (suffixes->st labeling-func suffixes alphabet)
  ...
  (foldl (lambda (ch acc)
           (let* (
                  (suff-ch (get-ch-words suffixes ch)))
             (if (null? suff-ch) acc
                 (let* (
                        (result (labeling-func suff-ch))
                        (label (list (car result)))
                        (rest (cdr result))
                        )
                   (append acc (list (append label (suffixes->st labeling-func rest alphabet))))
                 )))
           )
         '() alphabet)))
